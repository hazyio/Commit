...
- **Compose** — prefer stateless, reusable composables; hoist state up rather than holding it inside UI components
- **Naming** — match existing conventions in the codebase (e.g. `CommitTheme`, `CommitTypography` for theme-related declarations)
- **Formatting** — run Android Studio's default formatter (`Code → Reformat Code`) before committing
